...
func TestEveryPlainReportGetsAnUntrustedOrchestratorTurn(t *testing.T) {
	...
	if !strings.Contains(prompt, "ordinary progress") || !strings.Contains(prompt, "Available task tools remain enabled") || !strings.Contains(prompt, "active iMessage thread thread-opaque") || !strings.Contains(prompt, noUserReplyMarker) {
